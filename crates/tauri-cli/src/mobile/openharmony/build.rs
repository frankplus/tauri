...
pub fn command(options: Options, noise_level: NoiseLevel) -> Result<PathBuf> {
  ...
  run(options, &tauri_config, &interface, &app, noise_level)
}

pub fn run(
  options: Options,
  tauri_config: &crate::helpers::config::ConfigHandle,
  _interface: &AppInterface,
  app: &cargo_mobile2::config::app::App,
  _noise_level: NoiseLevel,
) -> Result<PathBuf> {
  ...
  let status = std::process::Command::new("cargo")
    .args([
      ...
      "--release",
    ]) // Force release for now as per reference
    .status()
  ...
  if !openharmony_dir.exists() {
    return Err(crate::Error::GenericError(format!(
      "OpenHarmony project directory not found at {}",
      openharmony_dir.display()
    )));
  }
  ...
  let mut lib_src = tauri_path
    .join("target/aarch64-unknown-linux-ohos/release")
    .join(&file_name);

  if !lib_src.exists() {
    // Fallback for workspace usage (e.g. basic-app in repo)
    let workspace_path = tauri_path
      .join("../../../target/aarch64-unknown-linux-ohos/release")
      .join(&file_name);
  ...
  std::fs::copy(&lib_src, &lib_dst).map_err(|e| {
    crate::Error::GenericError(format!("Failed to copy lib: {}", e))
  })?;
  ...
  let libcpp_src = PathBuf::from(format!(
    "{}/18/native/llvm/lib/aarch64-linux-ohos/c++/libc++_shared.so",
    sdk_base
  ));
  ...
  if libcpp_src.exists() {
    ...
    eprintln!(
      "Warning: libc++_shared.so not found at {}. Skipping copy.",
      libcpp_src.display()
    );
  }
